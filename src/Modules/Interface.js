...
export const Interface = (() => {
    ...
    const removeToDo = (toDo) => {
        ...
        _todayToDos = _todayToDos.filter(title => !(title.getTitle() == toDo));
    ...
    const initStorage = () => {
        ...
        if (_storageAvailable('localStorage')) {
            if(localStorage.length) {
                ...
                let newTodo = ToDo('Wake up', 'Get up now.', '23.06.1995', 'Medium', '1995-06-23', 'Daily Routine');
                _allToDos.push(newTodo);
                newTodo = ToDo('Brush teeth', 'Do it Well, do it often.', '23.06.1995', 'High', '1995-06-23', 'Daily Routine');
                _allToDos.push(newTodo);
                newTodo = ToDo('Make this app', "It won't make itself.", format(new Date(), 'dd.MM.yyyy'), 'High', '2021-05-15', 'The Odin Project');
                _allToDos.push(newTodo);
